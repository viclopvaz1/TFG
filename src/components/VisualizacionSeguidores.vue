<template>
    <div>
        <b-card class="overflow-auto" style="display: block; background-color: #f7f5f6; border-radius: 10px; max-height: 65vh">
            <div v-if="seguidores.length == 0" style="text-align: center">
                <b-card-text>Este profesor aun no tiene seguidores.</b-card-text>
            </div>
            <b-card v-for="(profesor, key) in seguidores" :key="key" 
